...
import DesignerForm2 from "../Components/DesignerForm2";
import Nav6 from '../Components/Nav/Nav6'
import { getDesigners, createDesigner, updateDesigner, deleteDesigner } from "../api";
import "../style.css";

const DesignerBackend = () => {
        ...
        <tbody>
          {filteredDesigners.map(d => (
            <tr key={d._id}>
              ...
              <td>{d.type}</td>
              <td>
                <button onClick={() => { setEditing(d); setShowForm(true); }}>✏</button>
...
export default DesignerBackend;